...
```
```{r decipher-oncotype-markers, echo=FALSE, message=FALSE}
decipher <- tibble::tribble(
    ~name, ~ensemble, ~invert,
    "LASP1", "ENSG00000002834", FALSE,
    "IQGAP3", "ENSG00000183856", FALSE,
    "NFIB", "ENSG00000147862", FALSE,
    "S1PR4", "ENSG00000125910", FALSE,
    "THBS2", "ENSG00000186340", FALSE,
    "ANO7", "ENSG00000146205", FALSE,
    "PCDH7", "ENSG00000169851", FALSE,
    "MYBPC1", "ENSG00000196091", TRUE,
    "EPPK1", "ENSG00000261150", FALSE,
    "PBX1", "ENSG00000185630", FALSE,
    "NUSAP1", "ENSG00000137804", FALSE,
    "ZWILCH", "ENSG00000174442", FALSE,
    "UBE2C", "ENSG00000175063", FALSE,
    "CAMK2N1", "ENSG00000162545", FALSE,
    "RABGAP1", "ENSG00000011454", FALSE,
    "TNFRSF19", "ENSG00000127863", TRUE
)

oncotype <- tibble::tribble(
    ~name, ~ensemble, ~invert,
    "BGN", "ENSG00000182492", FALSE,
    "COL1A1", "ENSG00000108821", FALSE,
    "SFRP4", "ENSG00000106483", FALSE,
    "FLNC", "ENSG00000128591", FALSE,
    "GSN", "ENSG00000148180", FALSE,
    "TPM2", "ENSG00000198467", FALSE,
    "GSTM2", "ENSG00000213366", FALSE,
    "TPX2", "ENSG00000088325", FALSE,
    "LAMB3", "ENSG00000196878", FALSE,
    "FAM13C", "ENSG00000148541", FALSE,
    "KLK2", "ENSG00000167751", TRUE,
    "AZGP1", "ENSG00000160862", TRUE,
    "SRD5A2", "ENSG00000277893", FALSE,
    "DUSP1", "ENSG00000120129", FALSE,
    "FOS", "ENSG00000170345", FALSE
)

pronostic_markers <- bind_rows(decipher, oncotype) %>%
    dplyr::filter(ensemble %in% colnames(disease))

disease <- disease %>%
    mutate(
        inv_prognostic = forcats::fct_recode(
            prognostic, "1" = "0", "0" = "1"
        ) %>% fct_relevel("0")
    )
...
results <- tibble(
    ...
    np_auc = double(),
    fp_auc = double(),
    curve_shape = character()
)
for (row in seq_len(nrow(pronostic_markers))) {
    response <- "prognostic"
    predictor <- pronostic_markers[[row, 2]]
    if (pronostic_markers[row, 3] == TRUE) {
        response <- "inv_prognostic"
    }
    result <- summarize_predictor(
        data = disease,
        predictor = .data[[predictor]],
        response = all_of(response),
        ratio = "tpr",
        threshold = 0.9
    ) %>% bind_cols(list(identifier = predictor))
